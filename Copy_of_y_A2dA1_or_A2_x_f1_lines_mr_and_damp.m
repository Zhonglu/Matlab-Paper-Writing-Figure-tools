...
for im=mi
    for ia=ai
        for idamp=dampi
            close all
            clearvars -except iG im mi Gi A2switch rstr dampi idamp ia ai
            ...
            if idamp==0
                lsgamdr=dir(['g','*','a',astr,'m',mstr,'r',rstr]);
            else
                dampstr=num2str(idamp,['%10.2','f']);
                lsgamdr=dir(['g','*','a',astr,'m',mstr,'d',dampstr,'r',rstr]);
            end
            ...
            for j=1:n0
                ...
                if gamdr(1)~=or(0.3,0.5,0.7,0.9);cycle;end
                if idamp==0
                    gamdr=[gamdr,0];
                    gamdr(5)=gamdr(5);
                    gamdr(4)=0;
                ...
                sgamall=sgamall+1;
                gamdall(1:length(gamdr),sgamall)=gamdr;
                A1=gamdr(2);
                ...
                for i=1:n
                    ...
                    clear ymax ymin yamp yampds t v02 y gamdr;
                ...
                legendInfo{sleg} = ['$G=',num2str(gamdall(1,sgamall),['%10.1f']),', \zeta=',num2str(gamdall(4,sgamall),['%10.3f']),'$'];
                %end
                cd ../
            end
            ...
            gmin=num2str(min(gamdall(1,:)));gmax=num2str(max(gamdall(1,:)));
            a1min=num2str(min(gamdall(2,:)));a1max=num2str(max(gamdall(2,:)));
            mrmin=num2str(min(gamdall(3,:)));mrmax=num2str(max(gamdall(3,:)));
            ...
            tiname=['Across_G=',gmin,'-',gmax,'_A1=',a1min,'-',a1max,'_mr=',mrmin,'-',mrmax,'_damp=',num2str(idamp),'-',num2str(idamp)];
            if A2switch
