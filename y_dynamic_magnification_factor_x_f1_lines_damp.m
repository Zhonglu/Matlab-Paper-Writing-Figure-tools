...
for iA=Ai
    for im=mi
        for iG=Gi
            ...
            A2switch=0; % A2=1 or mag_fac=0 or both
            ...
            if A2switch
                ...
                disp('drawing magnification factor graph')
            end
            ...
            for j=1:n0
                ...
                cd(lsgamr(j).name)
                Lf1=length(lsf1);
                f1=zeros(1,Lf1);A2=zeros(1,Lf1);F0r=zeros(1,Lf1);Beta=zeros(1,Lf1);
                ...
                for i=1:n
                    ...
                    nperiod=floor(Lt/(period1n));
                    ystart=floor(Lt/3);
                    F0r(i)=0.5*(max(y(ystart:Lt))-min(y(ystart:Lt)));%(max in 70 period - min)/2
                    ...
                    if A2switch==false
                        %Beta is dynamic manification factor
                        Beta(i)=A2(i)*2*(pi^3)*mr/F0r(i);
                    end
                ...
                saryaxim=saryaxim+1;
                if A2switch==false
                    if max(Beta)>yaxismax
                        [aryaxismax(sgamall),peaki]=max(Beta);
                        %aryaxis... denotes the range of window to zoom-in
                        %to see the peak clearly
                        if peaki >= length(Beta) %peak at end
                            aryaxismin(sgamall)=min(Beta((peaki-1):(peaki)));
                            arxaxismax(sgamall)=f1(peaki);
                            arxaxismin(sgamall)=f1(peaki-1)-(f1(peaki)-f1(peaki-1));
                        elseif peaki<=1 %peak at start
                            aryaxismin(sgamall)=min(Beta((peaki):(peaki+1)));
                            arxaxismax(sgamall)=f1(peaki);
                            arxaxismin(sgamall)=f1(peaki)-(f1(peaki+1)-f1(peaki));
                        else %peak in middle
                            aryaxismin(sgamall)=min(Beta((peaki-1):(peaki+1)));
            ...
            if A2switch
                ...
                printpdf(gcf,[tiname,'_mag_fac_overall',formatstr]);
                saveas(gcf,[tiname,'_mag_fac.fig'])
            end
            ...
            %printpdf(gcf,[tiname,'_mag_fac','.pdf']);
            ...
            if A2switch
                ...
                printpdf(gcf,[tiname,'_mag_fac_zoom',formatstr]);
            end
            %% figure 3
            set(h,'markersize',15);
            ...
            if A2switch
                ...
                printpdf(gcf,[tiname,'_mag_fac_details',formatstr]);
            end
            %%
        end
    end
