...
for iG=Gi
    for ia=ai
        for idamp=dampi
            ...
            if idamp==0
                lsgamdr=dir(['g',gstr,'a',astr,'m','*','r',rstr]);
            else
                dampstr=num2str(idamp,['%10.2','f']);
                lsgamdr=dir(['g',gstr,'a',astr,'m','*','d',dampstr,'r',rstr]);
            end
            ...
            for j=1:n0
                ...
                legendInfo{sleg} = ['$m^*=',num2str(gamall(3,sgamall),['%10.1f']),', \zeta=',num2str(gamall(4,sgamall),['%10.1f']),'$'];
                %end
                cd ../
            end
