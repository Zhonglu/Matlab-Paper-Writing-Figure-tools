...
global autocut_switch
global cutlength
autocut_switch=1;% 0 is to turn off autocut
cutlength=2;%first periods to cut off
syscount=0;%counter to go throught diff directories
for syscount=1:2
    ...
    list0=dir('g*a*m*d*e*r*'); %9326-9340
    n0=length(list0);
    s=0;resc=0;
    ...
    for j=1:n0 %1:n0
        ...
        for i=1:n % i=1:n
            ...
            ylimthl=[];ylimthr=[];
            mr=m;
            [err,stperiods,cphs(i),yphs(i)]=checkplot_CY_and_Y(fdir,zpf,start,pdir,ttl_p,pname,ylimfftl,f1,nT,ylimthl,A1,ylimfftr,ylimthr,mr);
            if err~=-1
                saveas(gcf,[ttl,'.fig'])
            end
            resc=resc+1;
...
%%
%clc
function [err,stperiods,cphs,yphs]=checkplot_CY_and_Y(fdir,zpf,start,pdir,ttl_p,pname,ylimfftl,f1,nT,ylimthl,A1,ylimfftr,ylimthr,mr)
global autocut_switch
setlatex
...
%% Verification config
condit=0;%f1==1.00; %condition to trigger verification plotting
fdname='Verification';
...
if condit
    ...
    %Cyt=Cyt+r1';
    %% functions to model Cy, generated by Eureqa
    ...
    t1_pre=t1;y1_pre=y2;
    if autocut_switch; [t1,y1,t2,y2]=autocut_for_irregular(f1,t1,y1,t2,y2);end
    %[Cyt,Cy2_sin,t2_sin,y2_sin]=autocut_for_irregular(f1,Cyt,Cy2_sin,t2_sin,y2_sin);
    %plot fft of input
    figure(4);hold on;
    [f, P1]=fftzp2(t1,y1);
...
if ~exist('pdir','var') || isempty(pdir); else; cd(pdir);
    ...
    saveas(gcf,[pname,'_',note1,'_Time_history_all_used.fig'])
end
%% FTT plot below
...
%figure(7)
yphs=0;
%[f, P1, yphs]=fftzp2(t2,y2);plot(f,P1,'color','r','LineStyle','--');
