...
close all
    ...
    A2switch=0; % A2=1 or A2dG=0 or both
    ...
    for j=1:n0
       ...
       % %tn=length(v02)/5;%Number of lines plotted
        ...
        if mrswitch 
            ...
            legendInfo{sleg} = ['$G=$',num2str(gamall(1,sgamall),4)];
        end
        cd ../
    end
    ...
    if A2switch
        ...
        ylabel('$A_2*G$','Interpreter','LaTex','FontSize',20);
    end
...
'hexagram'};
    ...
    tiname=['Across_G_G=',gmin,'-',gmax,'_A1=',a1min,'-',a1max,'_mr=',mrmin,'-',mrmax];
    if A2switch
        printpdf(gcf,[tiname,'_A2_overall','.eps']);
        saveas(gcf,[tiname,'_A2.fig'])
    else
        printpdf(gcf,[tiname,'_A2dG_overall','.eps']);
        saveas(gcf,[tiname,'_A2dG.fig'])
    end
    ...
    if A2switch
        printpdf(gcf,[tiname,'_A2_zoom','.pdf']);
    else
        printpdf(gcf,[tiname,'_A2dG_zoom','.pdf']);
    end
    %% figure 3
    set(h,'markersize',15);
    ...
    %tiname=['G=',gmin,'-',gmax,'_A1=',a1min,'-',a1max,'_mr_max=',mrmin,'-',mrmax];
    if A2switch
        printpdf(gcf,[tiname,'_A2_details','.pdf']);
    else
        printpdf(gcf,[tiname,'_A2dG_details.pdf']);
    end
    %%
