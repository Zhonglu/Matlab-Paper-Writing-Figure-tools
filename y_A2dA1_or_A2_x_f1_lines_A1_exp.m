...
close all
    clearvars -except iG im mi Gi
    
    
    sysdir
    gstr=num2str(iG,['%10.1','f']);
    mstr=num2str(im,['%10.1','f']);
    lsgamr=dir(['g',gstr,'a*m',mstr,'r*']);
    ...
    for j=1:n0
        ...
        %%
        count1=count1+1;
        f1=zeros(1,Lf1);A2(1:Lf1,count1)=zeros(1,Lf1);A2dA1=zeros(1,Lf1);
        ...
        for i=1:n
            ...
            ystart=floor(Lt/2);
            A2(i,count1)=0.5*(max(y(ystart:Lt))-min(y(ystart:Lt)));%(max in 70 period - min)/2
        ...
        saryaxim=saryaxim+1;
        if A2switch==false
            if max(A2dA1(:,count1))>yaxismax
                [aryaxismax(sgamall),peaki]=max(:,count1);
                aryaxismin(sgamall)=min(A2dA1((peaki-1):(peaki+1),count1));
                arxaxismax(sgamall)=f1(peaki+1);
                arxaxismin(sgamall)=f1(peaki-1)-(f1(peaki+1)-f1(peaki-1));
            end
        else
            if max(A2,count1)>yaxismax
       ...
       % %tn=length(v02)/5;%Number of lines plotted
        ...
        if mrswitch 
            ...
            legendInfo{sleg} = ['$A_1=$',num2str(gamall(2,sgamall),4)];
        end
        cd ../
    end
...
'hexagram'};
    ...
    tiname=['Across_A1_G=',gmin,'-',gmax,'_A1=',a1min,'-',a1max,'_mr=',mrmin,'-',mrmax];
    if A2switch
