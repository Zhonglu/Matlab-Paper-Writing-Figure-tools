...
for j=1:n0
    ...
    for i=1:n % i=1:n
        ...
        hold on;
        yyaxis right
        plot(f01((Nall-5*NT1):Nall,1),f01((Nall-5*NT1):Nall,3))%,'color','black')
        hold on;
        plot(f02((Nall-5*NT1):Nall,1),f02((Nall-5*NT1):Nall,3))%,'color','green')
        
        legendInfo{1} = ['Cylinder 1 disp'];
        legendInfo{2} = ['Cylinder 2 disp'];
        legendInfo{3} = ['Cylinder 2 force'];
        legendInfo{4} = ['Cylinder 2 force'];
        legend(legendInfo)
        ...
        mkdir(['plotting_force_check/',list0(j).name])
        cd (['plotting_force_check/',list0(j).name])
        print('-dpng','-r500',['Plotting_force_',tiname,'.png']);
        cd ('../')
        cd ('../')
        %close all
    end
